import pandas as pd
import numpy as np
from ascof.Mental_Health_Annex.ONEF import average_numerator
...
"""This creates only the 1H aggregations for the full year in review, alongside with its average score as well as its gender breakdowns per month."""


cleaned_1H_indicator = cleaning_months_together(months_together_input)


def clean_indicator_oneh(cleaned_df):
    filtered_indicator = cleaned_df.loc[
        (cleaned_1H_indicator["METRIC"] == "1H NUMERATOR")
        | (cleaned_1H_indicator["METRIC"] == "1H SCORE")
        | (cleaned_1H_indicator["METRIC"] == "DENOMINATOR")
    ]
...
def suppressed_num_denom_oc(averaged_data):
    ...
    old = ["1H NUMERATOR", "1H SCORE", "DENOMINATOR"]
    for o in old:
...
def rename_values(tidy_up_df):
    tidy_up_df["METRIC"] = tidy_up_df["METRIC"].replace(
        {"1H SCORE": "OUTCOME", "1H NUMERATOR": "NUMERATOR"}
    )
    return tidy_up_df


def pivotted_oneh(super_clean):
...
def oneh_main():
    ...
    cleaned_oneh = clean_indicator_oneh(clean)
    merge = merge_council_type_region(cleaned_oneh, source_data)
    ...
    england = clean_england(cleaned_oneh)
